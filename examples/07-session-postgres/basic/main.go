// Package main 演示使用 PostgreSQL 会话存储的非流式多轮对话。
//
// 功能特性：
//   - 基于 YAML 的配置和凭证管理
//   - 通过 ChatSessionStreamSync 进行非流式会话对话
//   - 在 PostgreSQL 存储中自动持久化历史记录
//
// 前置条件：
//   - 确保 PostgreSQL 正在运行并更新 connStr。
//   - 在 ../config.example.yaml 中配置有效凭证
//
// 使用方法：
//
//	go run basic.go
...
func main() {
	...
	connStr := "postgres://user:pass@localhost:5432/sessions?sslmode=disable"
	...
	// 步骤 4: 执行三轮对话会话以演示记忆
	sessionID := "postgresql-session-001"
