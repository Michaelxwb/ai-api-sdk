// Package main 演示使用 SQLite 会话存储的程序化认证多轮对话。
//
// 功能特性：
//   - 手动 Credential + ProviderConfig (no YAML)
//   - 通过 ChatSessionStreamSync 进行非流式会话对话
//   - 在 SQLite 存储中自动持久化历史记录
//
// 前置条件：
//   - 启用 CGO（github.com/mattn/go-sqlite3 所需）。
...
func main() {
	...
	resp1, err := cli.ChatSessionStreamSync(ctx1, providerName, sessionID, base.ChatRequest{
		...
		Messages: []base.Message{{Role: "user", Content: "用一段话介绍“Python”语言。"}},
	})
	if err != nil {
